...
def apply_fft_to_region(image, mask):
  ...
  for contour in contours:
    ...
    result_image[y:y+h, x:x +
                 w] = cv.cvtColor(fft_region, cv.COLOR_GRAY2BGR)  # 将结果转换回BGR
  ...
  for filename in os.listdir(input_folder):
    ...
    if os.path.isfile(input_path) and input_path.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff')):
      ...
      if img is not None:
        ...
        # 初次应用 Otsu's 阈值处理
        otsu_thresh_value, otsu_img = cv.threshold(
            cl1, 0, 255, cv.THRESH_BINARY + cv.THRESH_OTSU)
        ...
        filtered_img = cv.dilate(cleaned_otsu_img, kernel, iterations=1)
        connected_img = cv.morphologyEx(filtered_img, cv.MORPH_CLOSE, kernel)
        ...
        # 将蓝色图层应用到膨胀后的区域
        mask = filtered_img.astype(bool)
        img_color[mask] = cv.addWeighted(
            img_color, 1 - alpha, blue_layer, alpha, 0)[mask]

        # 在原图上绘制红色边界，包括内部的圈
        contours, hierarchy = cv.findContours(
            filtered_img, cv.RETR_CCOMP, cv.CHAIN_APPROX_SIMPLE)
        ...
        output_path_cleaned_otsu = os.path.join(
            output_folder, f'cleaned_otsu_{filename}')
        output_path_connected = os.path.join(
            output_folder, f'connected_{filename}')
        output_path_solid_polygon = os.path.join(
