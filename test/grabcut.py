...
output_folder = './ProcessedPictures_2'
    ...
    for filename in os.listdir(input_folder):
        ...
        if os.path.isfile(input_path) and input_path.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff')):
            ...
            if img is not None:
                ...
                # 矩形区域
                rect = (50, 50, img_color.shape[1] - 50, img_color.shape[0] - 50)

                # 使用初始的掩膜进行GrabCut
                mask[dilated_img == 255] = cv.GC_FGD
                mask[dilated_img == 0] = cv.GC_BGD

                cv.grabCut(img_color, mask, rect, bgd_model, fgd_model, 10, cv.GC_INIT_WITH_MASK)

                # 将结果掩膜处理为前景图像
                mask2 = np.where((mask == 2) | (mask == 0), 0, 1).astype('uint8')
                result = img_color * mask2[:, :, np.newaxis]

                # 在原图上绘制红色边界
                contours, _ = cv.findContours(mask2, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
                img_with_contours = img_color.copy()
                cv.drawContours(img_with_contours, contours, -1, (0, 0, 255), 2)
                ...
                output_path_grabcut = os.path.join(output_folder, f'grabcut_{filename}')
                output_path_contours = os.path.join(output_folder, f'contours_{filename}')
                ...
                cv.imwrite(output_path_grabcut, result)
                cv.imwrite(output_path_contours, img_with_contours)
            else:
